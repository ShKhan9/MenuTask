...
class MainControllerS : NSObject {
    ...
    static func Get(controller:MainViewControllerS,url:String,parameters1:Parameters,headers1:HTTPHeaders,showLoading:Bool = true,completion:@escaping((Data?) -> ()))  {
        ...
        Alamofire.request(url,method:.get,parameters:parameters1,headers:head).responseData { (response ) in
            // validate response
            ...
            case 200 :
                if response.result.value != nil{
                    print("success")
                    print(try! JSON(data: response.result.value!))
